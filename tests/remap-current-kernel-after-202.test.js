...
test('fresh remap of low-coupling kernel candidates after PR 202', () => {
  ...
  console.log('FRESH_REMAP_BASE|ce2b8ae9ceb9662b33891051ff33338099d1f914');
  console.log('FRESH_REMAP_BEGIN');
  for (const row of rows.slice(0, 160)) console.log('FRESH_REMAP|' + JSON.stringify(row));
  console.log('FRESH_REMAP_END');

  console.log('FRESH_REMAP_EXCLUDED_BEGIN');
  for (const row of excluded.slice(0, 120)) console.log('FRESH_REMAP_EXCLUDED|' + JSON.stringify(row));
  console.log('FRESH_REMAP_EXCLUDED_END');

  console.log('FRESH_REMAP_INFRA_ONLY_BEGIN');
  for (const row of infraOnly.slice(0, 120)) console.log('FRESH_REMAP_INFRA_ONLY|' + JSON.stringify(row));
